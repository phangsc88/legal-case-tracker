...
from db.queries import (
    ...
    db_add_task_to_case, db_delete_task, db_update_template_type_name, db_fetch_single_template_type
)
...
def ensure_default_admin():
    with get_db_connection() as conn:
        ...
        if user_count == 0:
            ...
            created = db_add_user(username="admin", password="1234", privilege="Admin")
            if created:
                print("✅ Default admin 'admin' with password '1234' created.")
            else:
...
def build_user_management_layout(privilege: str):
    ...
    if not users_df.empty:
        users_df['reset_action'] = "Reset Password"
        users_df['delete_action'] = "Delete"
    else:
        users_df = users_df.assign(reset_action=[], delete_action=[])
    ...
    return html.Div([
        ...
        dash_table.DataTable(
            ...
            columns=[
                ...
                {'name': 'Reset Password', 'id': 'reset_action'},
                {'name': 'Delete User', 'id': 'delete_action'}
            ],
            data=users_df.to_dict('records'),
            **DATATABLE_STYLE_DARK,
            style_cell_conditional=[
                {'if': {'column_id': 'reset_action'}, 'cursor': 'pointer', 'textDecoration': 'underline',
                 'color': DARK_THEME["colors"]["blue"][5]},
                {'if': {'column_id': 'delete_action'}, 'cursor': 'pointer', 'textDecoration': 'underline',
                 'color': DARK_THEME["colors"]["red"][5]}
            ]
        ),
        dbc.Modal([
            dbc.ModalHeader(dbc.ModalTitle("Reset User Password")),
            dbc.ModalBody([
                dcc.Store(id='reset-user-id-store'),
                html.P(id='reset-password-username-text'),
        ...
        dbc.Modal([
            ...
            dbc.ModalBody([
                ...
                html.P(id='delete-user-confirm-text')
            ]),
            dbc.ModalFooter(dmc.Group([
                dmc.Button("Cancel", id='delete-user-cancel-button', variant="outline"),
...
def build_template_type_list_items(privilege: str):
    ...
    for tt in template_types:
        ...
        if is_admin:
            buttons = [
                dmc.Button("Edit", id={'type': 'edit-template-type-btn', 'index': tt['template_type_id']},
                           variant="subtle", size="xs"),
                dmc.Button("Delete", id={'type': 'delete-template-type-btn', 'index': tt['template_type_id']},
                           variant="subtle", size="xs", color="red"),
            ]
            action_buttons_col = dbc.Col(dmc.Group(buttons), width="auto", className="d-flex align-items-center")
            row_children.append(action_buttons_col)

        item_row = dbc.Row(row_children, align="center", justify="between")
        items.append(item_row)

    return items


def build_template_tasks_container(template_id: int, privilege: str):
    tasks_df = db_fetch_tasks_for_template(template_id)
    is_admin = (privilege == 'Admin')
    ...
    add_task_form = dmc.Card(withBorder=True, children=[
        dmc.Title("Add New Task To Template", order=4, mb="md"),
        dbc.Row([
            dbc.Col(dmc.NumberInput(
                label="Seq #", id="new-task-seq", required=True,
                allowDecimal=True, decimalScale=2
            ), width=2),
            dbc.Col(dmc.TextInput(label="Task Name", id="new-task-name", required=True), width=4),
            dbc.Col(dmc.Select(
                label="Default Status", id="new-task-status",
                data=['Not Started', 'In Progress', 'On Hold', 'Completed'],
                value='Not Started', required=True
            ), width=3),
            dbc.Col(dmc.NumberInput(label="Days Offset", id="new-task-offset"), width=3)
        ]),
        dmc.Textarea(
            label="Documents Required", id="new-task-documents",
            autosize=True, minRows=2, mt="sm"
        ),
        dmc.Button("Add Task", id="add-task-to-template-button", mt="md")
    ], className="mt-4") if is_admin else None

    return html.Div([table, add_task_form])
...
def build_templates_layout(privilege: str):
    ...
    return html.Div([
        ...
        dcc.Store(id='template-to-delete-store'),
        dcc.Store(id='template-to-edit-store'),
        ...
        dbc.Modal([
            dbc.ModalHeader(dbc.ModalTitle("Confirm Deletion")),
            dbc.ModalBody(
                "Are you sure you want to delete this template and ALL of its associated tasks? This action cannot be undone."),
            dbc.ModalFooter(dmc.Group([
                dmc.Button("Cancel", id="cancel-delete-template-btn", variant="outline"),
                dmc.Button("DELETE TEMPLATE", id="confirm-delete-template-btn", color="red")
            ]))
        ], id='delete-template-confirm-modal', is_open=False, centered=True),
        dbc.Modal([
            dbc.ModalHeader(dbc.ModalTitle("Edit Template Type Name")),
            dbc.ModalBody([
                dmc.TextInput(id="edit-template-type-name-input", label="New Name:", required=True)
            ]),
            dbc.ModalFooter(dmc.Group([
...
def build_tasks_table_component(case_id: int, privilege: str):
    ...
    if is_admin:
        tasks_df['delete'] = "Delete"
    else:
        tasks_df['delete'] = None
...
def build_remarks_display_component(case_id: int):
    ...
    return [dbc.Card(dbc.CardBody([dmc.Text(f"By {row['user_name']} on {row['timestamp']}", size="xs", c="dimmed"),
                                   dmc.Text(row['message'], c="dark.0")]), className="mb-2",
                     style={"backgroundColor": DARK_THEME['colors']['dark'][6]}) for _, row in remarks_df.iterrows()]
...
def build_case_detail_layout(case_id: int, username: str, privilege: str):
    ...
    due_date, start_date_text = db_fetch_case_due_date(case_id), f"Started: {case_info['start_date']}" if case_info.get(
        'start_date') else "Not Started Yet"
    completed_date_text, case_due_date_text = f"Completed: {case_info['completed_date']}" if case_info.get(
        'completed_date') else "", f"Case Due: {due_date}" if due_date else ""
    is_admin = (privilege == 'Admin')
    is_user_role = not is_admin

    return html.Div([
        dcc.Store(id='detail-case-id-store', data=case_id),
        dcc.Store(id='detail-task-id-store'),
        dcc.Store(id='attachment-task-id-store'),
        dcc.Store(id='task-to-delete-store'),
        html.Div(id='detail-alert-container'),
        dbc.Card(dbc.CardBody([
            dbc.Row([
                dbc.Col([html.H2(case_info['case_name'], id="detail-case-name"),
                        ...
                        width=8),
                dbc.Col([dmc.Text("Change Case Status:", size="sm"), dmc.Select(id='detail-case-status-dropdown',
                                                                                data=['Not Started', 'In Progress',
                                                                                      'On Hold', 'Completed'],
                                                                                value=case_info['status']),
                         dmc.Button("Update Status", id='update-status-button', fullWidth=True, mt="md")], width=4)
            ])
        ])),
        html.Div([
            html.H3("Tasks", className="text-center my-4"),
            dmc.Button("Add New Task", id="open-add-task-modal-button", className="mb-3") if is_admin else None
        ]),
        html.Div(id='detail-tasks-table-container', children=build_tasks_table_component(case_id, privilege)),
        dbc.Modal([
            dbc.ModalHeader("Edit Task"),
            dbc.ModalBody([
                dmc.TextInput(label="Task Name:", id='edit-task-name'),
                dmc.Select(label="Status:", id='edit-task-status',
                           data=['Not Started', 'In Progress', 'On Hold', 'Completed'], mt="sm"),
                dmc.TextInput(label="Documents Required:", id='edit-task-documents', disabled=True, mt="sm"),
                html.Div([dmc.Text("Task Start Date:", size="sm"),
                          dmc.DatePicker(id='edit-task-start-date', style={"width": "100%"})], className="mt-2"),
                html.Div([dmc.Text("Task Completed Date:", size="sm"),
                          dmc.DatePicker(id='edit-task-completed-date', style={"width": "100%"})], className="mt-2"),
                html.Div([
                    dmc.Text("Due Date:", size="sm", mt="sm"),
                    dmc.DatePicker(id='edit-task-due-date',
                                   style={"width": "100%", "display": "none" if is_user_role else "block"}),
                    dmc.TextInput(id='edit-task-due-date-display', disabled=True,
            ...
            dbc.ModalFooter(dmc.Group([dmc.Button("Cancel", id="cancel-edit-task-button", variant="outline"),
        ...
        dbc.Modal([
            ...
            dbc.ModalBody([
                dbc.Card(dbc.CardBody([html.H5("Upload New File"), dcc.Upload(id='upload-attachment', children=html.Div(
                    ['Drag and Drop or ', html.A('Select Files')]), style={'width': '100%', 'height': '60px',
                                                                           'lineHeight': '60px', 'borderWidth': '1px',
                                                                           'borderStyle': 'dashed',
                                                                           'borderRadius': '5px', 'textAlign': 'center',
                                                                           'margin': '10px 0'}, multiple=True)])),
        ...
        dbc.Modal([
            dbc.ModalHeader("Add New Task to Case"),
            dbc.ModalBody([
                dmc.TextInput(label="Task Name:", id="add-task-name-input", required=True),
                dmc.TextInput(label="Documents Required:", id="add-task-docs-input", mt="sm"),
                dmc.NumberInput(
                    label="Day Offset (for automatic due dates):",
                    id="add-task-offset-input",
                    mt="sm",
                    min=0,
                    step=1,
                    placeholder="e.g., 7 for 7 days after case start"
                ),
                dmc.Select(
                    label="Insert Position:",
                    id="add-task-position-select",
                    data=[],
                    required=True,
                    mt="sm",
            ...
            dbc.ModalFooter(dmc.Group([
                dmc.Button("Cancel", id="cancel-add-task-button", variant="outline"),
                dmc.Button("Add Task", id="save-add-task-button")
            ]))
        ], id="add-task-modal", is_open=False, centered=True),
        dbc.Modal([
            dbc.ModalHeader("Confirm Task Deletion"),
            dbc.ModalBody(
                "Are you sure you want to delete this task? This will also remove all its attachments and cannot be undone."),
            dbc.ModalFooter(dmc.Group([
                dmc.Button("Cancel", id="cancel-delete-task-button", variant="outline"),
                dmc.Button("DELETE TASK", id="confirm-delete-task-button", color="red")
            ]))
        ], id="delete-task-modal", is_open=False, centered=True),
...
@app.callback(Output('page-content', 'children'), Output('navbar-container', 'children'), Input('url', 'pathname'),
...
def main_router_and_navbar(pathname, session_data):
    ...
    links = [{"href": "/", "label": "Home"}, {"href": "/templates", "label": "Manage Templates"},
             {"href": "/calendar", "label": "Calendar"}, {"href": "/dashboard", "label": "Dashboard"},
             {"href": "/date-report", "label": "Reports"}]
    ...
    user_menu = dmc.Menu([dmc.MenuTarget(dmc.Button(f"{username} ({privilege})", variant="outline")),
                          dmc.MenuDropdown([dmc.MenuItem("Logout", href="/logout")])]) if is_authenticated else None
    navbar = dbc.Navbar(dbc.Container(
        [dbc.NavbarBrand("Legal Case Tracker", href="/"), dbc.Nav(nav_items, navbar=True, className="ms-auto"),
         html.Div(user_menu, className="ms-3") if user_menu else None]), color=DARK_THEME["colors"]["dark"][7],
    ...
    route_map = {'/user-management': lambda: build_user_management_layout(privilege),
                 '/templates': lambda: build_templates_layout(privilege), '/calendar': build_calendar_layout,
                 '/dashboard': build_dashboard_layout, '/date-report': build_date_report_layout}
    ...
    if pathname and pathname.startswith('/case/'):
        try:
            ...
            return build_case_detail_layout(case_id, username,
                                            privilege), navbar
...
@app.callback(Output('session-store', 'data', allow_duplicate=True), Output('url', 'pathname', allow_duplicate=True),
              Output('login-alert', 'children'), Input('login-button', 'n_clicks'), State('login-username', 'value'),
...
def handle_login(n_clicks, username, password):
    if not username or not password: return dash.no_update, dash.no_update, dbc.Alert(
        "Please enter username and password", color="warning")
    user_data = db_get_user(username)
    if user_data and check_password(user_data['password_hash'], password): return {'is_authenticated': True,
                                                                                   'username': username,
                                                                                   'privilege': user_data[
                                                                                       'privilege']}, '/', None
...
@app.callback(Output('session-store', 'data', allow_duplicate=True), Input('url', 'pathname'),
              State('session-store', 'data'), prevent_initial_call=True)
def handle_logout(pathname, session_data):
    if pathname == '/logout' and (session_data or {}).get('is_authenticated'): return {'is_authenticated': False,
                                                                                       'username': None,
                                                                                       'privilege': None}
    raise PreventUpdate


@app.callback(Output("forgot-password-modal", "is_open"), Input("forgot-password-link", "n_clicks"),
...
@app.callback(Output('url', 'pathname', allow_duplicate=True),
              Input({'type': 'view-case-btn', 'index': ALL}, 'n_clicks'), prevent_initial_call=True)
...
@app.callback(Output('url', 'pathname', allow_duplicate=True),
              Input({'type': 'calendar-view-case-btn', 'index': ALL}, 'n_clicks'), prevent_initial_call=True)
...
@app.callback(Output('url', 'pathname', allow_duplicate=True),
              Input({'type': 'calendar-daily-view-btn', 'index': ALL}, 'n_clicks'), prevent_initial_call=True)
def calendar_daily_navigation(n_clicks):
    if not any(n_clicks): raise PreventUpdate
    return f"/case/{ctx.triggered_id['index']}"


@app.callback(Output('url', 'pathname', allow_duplicate=True), Input('report-table', 'active_cell'),
...
@app.callback(Output('user-management-alert', 'children', allow_duplicate=True),
              Output('users-table', 'data', allow_duplicate=True), Input('add-user-button', 'n_clicks'),
              State('add-user-username', 'value'), State('add-user-password', 'value'),
              State('add-user-privilege', 'value'), prevent_initial_call=True)
def add_user(n_clicks, username, password, privilege):
    ...
    if db_add_user(username, password, privilege):
        ...
        if not updated_users_df.empty:
            updated_users_df['reset_action'] = "Reset Password"
            updated_users_df['delete_action'] = "Delete"
...
def open_user_action_modals(active_cell, data):
    ...
    if column_id not in ['reset_action', 'delete_action']:
        raise PreventUpdate
    ...
    if column_id == 'reset_action':
        return True, user_id, f"Enter new password for user: {username}", False, dash.no_update, dash.no_update

    elif column_id == 'delete_action':
        confirm_text = f"Are you sure you want to permanently delete the user '{username}'?"
        return False, dash.no_update, dash.no_update, True, user_id, confirm_text

    raise PreventUpdate


@app.callback(Output('user-management-alert', 'children'),
...
def handle_reset_password(n_clicks, user_id, new_password):
    ...
    if db_update_user_password(user_id, new_password): return dbc.Alert("Password reset successfully.",
                                                                        color="success"), False, ""
    return dbc.Alert("Failed to reset password.", color="danger"), False, ""


@app.callback(Output('reset-password-modal', 'is_open', allow_duplicate=True),
              Input('reset-password-cancel-button', 'n_clicks'), prevent_initial_call=True)
def cancel_reset_password(n_clicks):
...
@app.callback(
    ...
    Input('delete-user-confirm-button', 'n_clicks'),
    State('delete-user-id-store', 'data'),
    prevent_initial_call=True
)
def handle_delete_user(n_clicks, user_id):
    ...
    alert = dbc.Alert("User has been deleted successfully.", color="danger")
    return alert, updated_users_df.to_dict('records'), False


@app.callback(
    Output('delete-user-modal', 'is_open', allow_duplicate=True),
    Input('delete-user-cancel-button', 'n_clicks'),
    prevent_initial_call=True
)
def cancel_user_deletion(n_clicks):
    if not n_clicks:
        raise PreventUpdate
    return False


@app.callback(Output('case-list-container', 'children', allow_duplicate=True),
              Output('home-alert-container', 'children', allow_duplicate=True),
              Input('home-add-case-button', 'n_clicks'), State('session-store', 'data'),
              State('home-new-case-name', 'value'), State('home-new-case-status', 'value'),
              State('home-new-case-type', 'value'), prevent_initial_call=True)
def home_add_case(n_clicks, session_data, name, status, case_type):
    if not all([name, status, case_type]): return dash.no_update, dbc.Alert("All fields are required to add a case.",
                                                                            color="warning")
    new_case_id = db_add_case(name, status, case_type);
    db_populate_tasks_from_template(new_case_id, case_type)
    return build_cases_list_component(session_data.get('privilege')), dbc.Alert(f"Case '{name}' added successfully.",
                                                                                color="success")


@app.callback(Output('edit-case-modal', 'is_open'), Output('delete-case-modal', 'is_open'),
              Output('edit-case-id-store', 'data'), Output('delete-case-id-store', 'data'),
              Output('modal-edit-case-name', 'value'), Output('modal-edit-case-status', 'value'),
              Output('modal-edit-case-type', 'value'), Output('delete-case-confirm-text', 'children'),
              Input({'type': 'edit-case-btn', 'index': ALL}, 'n_clicks'),
              Input({'type': 'delete-case-btn', 'index': ALL}, 'n_clicks'), prevent_initial_call=True)
def open_case_modals(edit_clicks, delete_clicks):
    ...
    case_info = db_fetch_single_case(case_id)
    if 'edit-case-btn' in ctx.triggered_id['type'] and case_info:
        return True, False, case_id, dash.no_update, case_info['case_name'], case_info['status'], case_info[
            'case_type'], dash.no_update
    elif 'delete-case-btn' in ctx.triggered_id['type'] and case_info:
        return False, True, dash.no_update, case_id, dash.no_update, dash.no_update, dash.no_update, f"Delete '{case_info['case_name']}'? This will delete all associated tasks."
    return False, False, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update, dash.no_update


@app.callback(Output('case-list-container', 'children', allow_duplicate=True),
              Output('home-alert-container', 'children', allow_duplicate=True),
              Output('edit-case-modal', 'is_open', allow_duplicate=True), Input('save-edit-case-button', 'n_clicks'),
              State('session-store', 'data'), State('edit-case-id-store', 'data'),
              State('modal-edit-case-name', 'value'), State('modal-edit-case-status', 'value'),
              State('modal-edit-case-type', 'value'), prevent_initial_call=True)
def save_case_edit(n_clicks, session_data, case_id, name, status, case_type):
    if not all([name, status, case_type]): return dash.no_update, dbc.Alert("All fields are required.",
                                                                            color="warning"), True
    db_update_case(case_id, name, status, case_type)
    return build_cases_list_component(session_data.get('privilege')), dbc.Alert(f"Case '{name}' updated.",
                                                                                color="success"), False
...
@app.callback(Output('case-list-container', 'children', allow_duplicate=True),
              ...
              Input('confirm-delete-case-button', 'n_clicks'), State('session-store', 'data'),
              State('delete-case-id-store', 'data'), prevent_initial_call=True)
def confirm_case_delete(n_clicks, session_data, case_id):
    db_delete_case(case_id)
    return build_cases_list_component(session_data.get('privilege')), dbc.Alert("Case has been deleted.",
                                                                                color="danger"), False


@app.callback(Output('edit-case-modal', 'is_open', allow_duplicate=True),
              Output('delete-case-modal', 'is_open', allow_duplicate=True),
              Input('cancel-edit-case-button', 'n_clicks'), Input('cancel-delete-case-button', 'n_clicks'),
              prevent_initial_call=True)
...
@app.callback(
    ...
    Output('detail-task-id-store', 'data'),
    Output('edit-task-name', 'value'),
    Output('edit-task-status', 'value'),
    ...
    Output('edit-task-documents', 'value'),
    Output('delete-task-modal', 'is_open', allow_duplicate=True),
    Output('task-to-delete-store', 'data'),
...
@app.callback(Output('edit-task-modal', 'is_open', allow_duplicate=True), Output('detail-alert-container', 'children'),
              Output('detail-tasks-table-container', 'children'), Output('detail-case-status-dropdown', 'value'),
              Output('case-completed-date-display', 'children'), Output('start-date-display', 'children'),
              Output('case-due-date-display', 'children'), Input('save-edit-task-button', 'n_clicks'),
              State('session-store', 'data'), State('detail-task-id-store', 'data'),
              State('detail-case-id-store', 'data'), State('edit-task-name', 'value'),
              State('edit-task-status', 'value'), State('edit-task-due-date', 'value'),
              State('edit-task-start-date', 'value'), State('edit-task-completed-date', 'value'),
              State('detail-tasks-table', 'data'), prevent_initial_call=True)
def handle_edit_task_modal_actions(save_clicks, session_data, task_id, case_id, name, status, new_due_date, start_date,
    ...
    new_start_text = f"Started: {updated_case_info.get('start_date')}" if updated_case_info.get(
        'start_date') else "Not Started Yet"
    new_complete_text = f"Completed: {updated_case_info.get('completed_date')}" if updated_case_info.get(
        'completed_date') else ""
    ...
    return False, dbc.Alert(alert_msg, color="success", duration=5000), refreshed_table, updated_case_info.get(
        'status'), new_complete_text, new_start_text, new_due_text


@app.callback(Output('edit-task-modal', 'is_open', allow_duplicate=True), Input('cancel-edit-task-button', 'n_clicks'),
              prevent_initial_call=True)
...
@app.callback(Output('attachment-list-container', 'children', allow_duplicate=True),
              ...
              Input('upload-attachment', 'contents'), State('upload-attachment', 'filename'),
              State('attachment-task-id-store', 'data'), State('detail-case-id-store', 'data'),
              State('session-store', 'data'), prevent_initial_call=True)
def handle_file_upload(list_of_contents, list_of_names, task_id, case_id, session_data):
    ...
    return build_attachments_list(task_id), build_tasks_table_component(case_id, privilege), dbc.Alert(
        f"{len(list_of_names)} file(s) uploaded!", color="success")
...
def handle_file_delete(n_clicks, task_id, case_id, session_data):
    ...
    return build_attachments_list(task_id), build_tasks_table_component(case_id, privilege), dbc.Alert(
        "Attachment deleted.", color="danger")


@app.callback(Output('templates-alert-container', 'children', allow_duplicate=True),
              Output('template-type-list', 'children'), Input('add-template-type-button', 'n_clicks'),
              State('session-store', 'data'), State('new-template-type-name', 'value'), prevent_initial_call=True)
...
@app.callback(Output('template-tasks-container', 'children'), Input('selected-template-type-id-store', 'data'),
              State('session-store', 'data'))
...
@app.callback(Output('delete-template-confirm-modal', 'is_open'), Output('template-to-delete-store', 'data'),
              Input({'type': 'delete-template-type-btn', 'index': ALL}, 'n_clicks'),
              State('delete-template-confirm-modal', 'is_open'), prevent_initial_call=True)
def open_delete_template_modal(n_clicks, is_open):
    if not any(n_clicks): raise PreventUpdate
    return not is_open, ctx.triggered_id['index']


@app.callback(Output('delete-template-confirm-modal', 'is_open', allow_duplicate=True),
              Output('template-type-list', 'children', allow_duplicate=True),
              Output('templates-alert-container', 'children', allow_duplicate=True),
              Input('confirm-delete-template-btn', 'n_clicks'), Input('cancel-delete-template-btn', 'n_clicks'),
              State('template-to-delete-store', 'data'), State('session-store', 'data'), prevent_initial_call=True)
...
@app.callback(Output('add-task-modal', 'is_open'), Output('add-task-position-select', 'data'),
              Input('open-add-task-modal-button', 'n_clicks'), State('detail-case-id-store', 'data'),
              prevent_initial_call=True)
def open_add_task_modal(n_clicks, case_id):
    if not n_clicks: raise PreventUpdate
    tasks_df = db_fetch_tasks_for_case(case_id)
    options = [{'label': 'At the Beginning', 'value': '0'}]
    if not tasks_df.empty:
        options += [{'label': f"After: {row['task_name']}", 'value': str(row['task_id'])} for _, row in
                    tasks_df.iterrows()]
    return True, options


@app.callback(Output('add-task-modal', 'is_open', allow_duplicate=True),
              Output('detail-tasks-table-container', 'children', allow_duplicate=True),
              Output('detail-alert-container', 'children', allow_duplicate=True),
              Output('add-task-name-input', 'value'), Output('add-task-docs-input', 'value'),
              Output('add-task-offset-input', 'value'), Input('save-add-task-button', 'n_clicks'),
              State('session-store', 'data'), State('detail-case-id-store', 'data'),
              State('add-task-name-input', 'value'), State('add-task-docs-input', 'value'),
              State('add-task-offset-input', 'value'), State('add-task-position-select', 'value'),
              prevent_initial_call=True)
def save_new_task(n_clicks, session_data, case_id, name, docs, offset, position):
    if not all([name, position is not None]):
        return True, dash.no_update, dbc.Alert("Task Name and Position are required.",
    ...
    if privilege == 'Admin':
        ...
        db_add_task_to_case(case_id, name, "", docs, offset, position_int, username)
        refreshed_table = build_tasks_table_component(case_id, privilege)
        alert = dbc.Alert("Task added successfully!", color="success")
        return False, refreshed_table, alert, "", "", None

    return False, dash.no_update, dbc.Alert("You do not have permission.", color="danger"), "", "", None


@app.callback(Output('delete-task-modal', 'is_open', allow_duplicate=True),
              Output('detail-tasks-table-container', 'children', allow_duplicate=True),
              Output('detail-alert-container', 'children', allow_duplicate=True),
              Input('confirm-delete-task-button', 'n_clicks'), State('session-store', 'data'),
              State('task-to-delete-store', 'data'), State('detail-case-id-store', 'data'), prevent_initial_call=True)
def perform_task_deletion(n_clicks, session_data, task_id, case_id):
    if not n_clicks: raise PreventUpdate
    privilege = (session_data or {}).get('privilege')
    if privilege == 'Admin':
        db_delete_task(task_id)
        refreshed_table = build_tasks_table_component(case_id, privilege)
        alert = dbc.Alert("Task has been deleted.", color="danger")
        return False, refreshed_table, alert
    return False, dash.no_update, dbc.Alert("You do not have permission.", color="danger")


@app.callback(Output('add-task-modal', 'is_open', allow_duplicate=True), Input('cancel-add-task-button', 'n_clicks'),
              prevent_initial_call=True)
def cancel_add_task(n_clicks):
    if not n_clicks: raise PreventUpdate
    return False


@app.callback(Output('delete-task-modal', 'is_open', allow_duplicate=True),
              Input('cancel-delete-task-button', 'n_clicks'), prevent_initial_call=True)
def cancel_delete_task(n_clicks):
    if not n_clicks: raise PreventUpdate
    return False


@app.callback(Output('detail-alert-container', 'children', allow_duplicate=True),
              Input('update-status-button', 'n_clicks'), State('detail-case-id-store', 'data'),
              State('detail-case-status-dropdown', 'value'), State('session-store', 'data'), prevent_initial_call=True)
...
@app.callback(Output('remarks-display-area', 'children', allow_duplicate=True),
              Output('remark-message-textarea', 'value'),
              Output('detail-alert-container', 'children', allow_duplicate=True),
...
def add_remark_to_case(n, case_id, user_name, message):
    if not message or not message.strip(): return dash.no_update, dash.no_update, dbc.Alert('Remark cannot be empty.',
                                                                                            color='warning')
...
@app.callback(Output('tasks-for-selected-date', 'children'), Output('selected-date-header', 'children'),
...
def update_tasks_for_date(selected_date_str):
    ...
    sel = date.fromisoformat(selected_date_str);
    df = db_fetch_tasks_for_date(sel)
    header = f"Tasks for {sel.strftime('%B %d, %Y')}"
    if df.empty: return [dbc.Alert('No tasks due on this date.', color='info')], header
    items = [dbc.Alert([html.Strong(f"{r['case_name']}: "), html.Span(r['task_name']),
                        dmc.Button('View', id={'type': 'calendar-daily-view-btn', 'index': r['case_id']},
                                   variant='subtle', size='sm', className='float-end')], color='primary',
                       className='mb-2') for _, r in df.iterrows()]
    return items, header


@app.callback(Output('upcoming-overdue-tasks-table-container', 'children'),
...
@app.callback(Output('case-status-pie-chart', 'figure'), Output('case-performance-pie-chart', 'figure'),
              Output('task-status-bar-chart', 'figure'), Output('task-performance-bar-chart', 'figure'),
              Output('case-performance-by-type-bar-chart', 'figure'), Input('dashboard-generate-button', 'n_clicks'),
              State('dashboard-from-date', 'value'), State('dashboard-to-date', 'value'), prevent_initial_call=True)
def update_dashboard_charts(n_clicks, from_date, to_date):
    ...
    figs = [px.pie(cases_df, names='status', title='Case Status'),
            px.pie(cases_df, names='performance', title='Case Performance'),
            px.histogram(tasks_df, x='status', title='Task Status'),
            px.histogram(tasks_df, x='performance', title='Task Performance'),
            px.bar(cases_df.groupby(['case_type', 'performance']).size().reset_index(name='count'), x='case_type',
                   y='count', color='performance', barmode='group', title='Case Performance by Type')]
    for fig in figs: fig.update_layout(legend_title_text='')
    return figs


@app.callback(Output('report-output-container', 'children'), Input('report-generate-button', 'n_clicks'),
...
def generate_date_report(n_clicks, report_type, from_date, to_date):
    ...
    style = DATATABLE_STYLE_DARK.copy();
    style['style_cell_conditional'] = style.get('style_cell_conditional', []) + [
        {'if': {'column_id': 'action'}, 'cursor': 'pointer', 'text-decoration': 'underline',
         'color': DARK_THEME['colors']['blue'][5]}]
    return dash_table.DataTable(id='report-table', columns=cols, data=df.to_dict('records'), page_size=20, **style)


# Callback to open the edit template type modal
@app.callback(
    Output('edit-template-type-modal', 'is_open'),
    Output('template-to-edit-store', 'data'),
    Output('edit-template-type-name-input', 'value'),
    Input({'type': 'edit-template-type-btn', 'index': ALL}, 'n_clicks'),
    prevent_initial_call=True
)
def open_edit_template_type_modal(n_clicks):
    if not any(n_clicks):
        raise PreventUpdate

    clicked_id = ctx.triggered_id['index']
    current_name = db_fetch_single_template_type(clicked_id)

    if current_name is None:
        return False, dash.no_update, dash.no_update

    modal_data = {'id': clicked_id}
    return True, modal_data, current_name


# Callback to save the new template type name
@app.callback(
    Output('edit-template-type-modal', 'is_open', allow_duplicate=True),
    Output('template-type-list', 'children', allow_duplicate=True),
    Output('templates-alert-container', 'children', allow_duplicate=True),
    Input('save-edit-template-type-btn', 'n_clicks'),
    Input('cancel-edit-template-type-btn', 'n_clicks'),
    State('template-to-edit-store', 'data'),
    State('edit-template-type-name-input', 'value'),
    State('session-store', 'data'),
    prevent_initial_call=True
)
def handle_edit_template_type_saving(save_clicks, cancel_clicks, edit_data, new_name, session_data):
    if ctx.triggered_id == 'cancel-edit-template-type-btn':
        return False, dash.no_update, dash.no_update

    if not new_name or not new_name.strip():
        return True, dash.no_update, dbc.Alert("Name cannot be empty.", color="warning")

    template_type_id = edit_data['id']
    db_update_template_type_name(template_type_id, new_name)
